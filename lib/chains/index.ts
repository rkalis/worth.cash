// Chain metadata ported from @revoke.cash/core, trimmed to what a portfolio tracker needs.
//
// The chain entries and select lists are copied from upstream, with three deliberate differences:
//   1. RPC URLs that embed an API key use a `{ALCHEMY_API_KEY}` style placeholder instead of an
//      interpolated env var, because our keys are configured at runtime rather than at build time.
//      `Chain.getRpcUrls()` resolves them and drops any URL whose key is not configured.
//   2. The Covalent support type is gone. Ronin is synced through Etherscan instead, and Viction through its
//      public RPC, which serves browser requests and answers eth_getLogs over wide block ranges.
//   3. Helpers that only serve revoke.cash (slugs, add-network parameters, backend support types) are left out.
import { Chain, type DeployedContracts, type NativeCurrency, SupportType } from 'lib/chains/Chain';
...
// Order is loosely based on TVL (as per DeFiLlama). Make sure to update these lists when adding chains.
...
export const CHAINS = {
  ...
  [ChainId.Zircuit]: new Chain({
    ...
    rpc: {
      ...
      free: 'https://mainnet.zircuit.com',
    },
    deployedContracts: { multicall3: { address: MULTICALL_ADDRESS, blockCreated: 0 } },
    isOpStack: true,
  }),
...
// Unlike `getChainConfig`, this tolerates a chain id we have no configuration for, which happens when
// IndexedDB holds data written by an older version of the chain list.
export const getChainConfigSafe = (chainId: number): Chain | undefined => {
  return (CHAINS as Record<number, Chain | undefined>)[chainId];
};

// Takes any number, since it is also how stored chain ids are checked against the current chain list.
export const isSupportedChain = (chainId: number): boolean => {
  return Boolean(getChainConfigSafe(chainId)?.isSupported());
};
...
export const isMainnetChain = (chainId: DocumentedChainId): boolean => {
  return SUPPORTED_CHAINS.includes(chainId) && !isTestnetChain(chainId);
};
...
export const getChainName = (chainId: DocumentedChainId): string => {
  return getChainConfigSafe(chainId)?.getName() ?? `Chain ${chainId}`;
};
...
export const getChainLogsRpcUrls = (chainId: DocumentedChainId): string[] => {
  return getChainConfig(chainId).getLogsRpcUrls();
};

export const getChainLogo = (chainId: DocumentedChainId): string | undefined => {
  return getChainConfigSafe(chainId)?.getLogoUrl();
};
...
export const createViemPublicClientForChain = (
  ...
  return getChainConfig(chainId).createViemPublicClient(overrideUrl, blockNumber, httpOptions);
};

// Sorts chain ids by the curated order above, falling back to numeric order for anything unlisted.
export const chainIdSorter = (a: number, b: number) => {
  const aIndex = (ORDERED_CHAINS as readonly number[]).indexOf(a);
  const bIndex = (ORDERED_CHAINS as readonly number[]).indexOf(b);
